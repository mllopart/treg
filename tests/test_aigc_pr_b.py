...
def static_descriptor(*, fetch: bool = False) -> dict:
    result = ({"fetch": "vendor.result", "fetch_param": {"pathParams": "file_id"},
               "ttl_note": "9h"} if fetch else {"path": "task.content.url"})
    return {
        "id_from": "task_id",
        "poll": {"endpoint": "vendor.task.status", "param": {"queryParams": "task_id"}},
...
def test_async_wait_dynamic_url_enforces_hosts_and_fetches_by_terminal_id():
    ...
    terminal = response(200, {"task": {"status": "succeeded"}, "file_id": "file-9"})
    outcome = cli.await_async_task(
        descriptor,
...
def test_call_await_writes_only_terminal_raw_body_to_stdout(monkeypatch, capsys):
    ...
    terminal = response(200, {"task": {"status": "succeeded"}, "file_id": "file-7"})
...
def test_ingesters_are_byte_deterministic(tmp_path, monkeypatch):
    ...
    openrouter_payload = {"data": [{
        "id": "minimax/hailuo-test", "name": "Hailuo Test", "description": "Text-to-video test model",
        "supported_durations": [5], "supported_resolutions": ["720p"],
        "supported_aspect_ratios": ["16:9"], "pricing_skus": {"duration_seconds_720p": "0.1"},
    }]}
    replicate_model = {
